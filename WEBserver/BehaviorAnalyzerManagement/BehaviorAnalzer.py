...
class BehaviorAnalzer():
    ...
    # 기본 프로세스 Tree 가져오기
    def Get_Root_Process_Tree______(self, endpoint_type: SigmaTargetEndpoint, agent_id:str, root_process_id:str)->Optional[dict]:
        Tree = self.EDR_Manager.Get_Process_Tree(root_process_id=root_process_id)[0]
        if not Tree:
            return None
        
        return self.Tree_with_Sigma______(
    ...
    # Tree 이벤트에 분석결과 결합
    def Tree_with_Sigma______(self, endpoint_type: SigmaTargetEndpoint,Tree:dict, agent_id:str, root_process_id:str)->dict:
    ...
    # 재귀함수
    def Tree_with_Sigma_recursive____________(self, endpoint_type: SigmaTargetEndpoint, agent_id:str, root_process_id:str, Child:list[dict]):
        if len(Child) == 0:
            return None
        else:
    ...
    # 이벤트 -> 분석 결과 결합
    def Merge_Analysis_into_Event(self, endpoint_type: SigmaTargetEndpoint, event:dict, event_name:str, agent_id:str, root_process_id:str) -> dict:
        output = {}
